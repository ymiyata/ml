function compile_result(m, n, generate_data)
    if generate_data
        for i = m:n
            create_results(i);
        end
    ...
    sum_test_accuracy = 0;
    for i = m:n
        mat_filename = sprintf('rhc_result%d', i);
        load(mat_filename);
        sum_iterations = sum_iterations + output.iterations;
    ...
    fprintf(fh, 'Avg Training Time:       %d\n', sum_totaltime/n);
    fprintf(fh, 'Avg # of Iterations:     %d\n', sum_iterations/n);
...
function create_results(test_number)
    [x fval flag output net train_set test_set] = nn_usingrhc(3);
    mat_filename = sprintf('rhc_result%d.mat', test_number);
    train_accuracy = accuracy(net, x, train_set{1}, train_set{2});
    test_accuracy = accuracy(net, x, test_set{1}, test_set{2});
    save(mat_filename, 'x', 'fval', 'flag', 'output', 'net', 'train_set', 'train_accuracy', 'test_set', 'test_accuracy');
