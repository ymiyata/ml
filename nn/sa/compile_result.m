function compile_result(n, generate_data)
    ...
    sum_test_accuracy = 0;
    for i = 1:n
        mat_filename = sprintf('sa_result%d', i);
        load(mat_filename);
        sum_totaltime = sum_totaltime + output.totaltime;
...
function create_results(test_number)
    [x fval flag output net train_set test_set] = nn_usingsa(3);
    mat_filename = sprintf('sa_result%d', test_number);
