n = 30;
max_runs = 10;
optimal = four_peaks([ones(1, round(n*0.1)+1), zeros(1, round(n*0.9)-1)]);
...
for run = 1:max_runs
    ...
    [x, fval, flag, output] = patternsearch(@four_peaks, input, ...
        [], [],[],[], -Inf, Inf, options);
...
for population = pops
    ...
    for run = 1:max_runs
        ...
        [x, fval, flag, output, pop, score] = ga(@four_peaks, n, options);
...
for run = 1:max_runs
    ...
    input = round(rand(1, n));
    [x fval flag output] = simulannealbnd (@four_peaks, input, -Inf, Inf, options);
    results(run).x = x;
    results(run).fval = fval;
    results(run).error = abs(optimal - fval);
    results(run).evaluation = output.funccount;
